...
export class Game {
  ...
  public constructor(container: HTMLElement) {
    ...
    this.healthHudElement = document.createElement('div')
    this.healthHudElement.className = 'health-hud'
    this.container.appendChild(this.healthHudElement)

    this.gameOverElement = document.createElement('div')
    this.gameOverElement.className = 'game-over'
    this.gameOverElement.innerHTML = `
      <div class="game-over__panel">
        <div class="game-over__eyebrow">ELEMENTAL BOMBER</div>
        <h1 class="game-over__title">GAME OVER</h1>
        <p class="game-over__message">The fire has gone out...</p>
        <button class="game-over__restart" type="button">
          RESTART
        </button>
      </div>
    `
    ...
    const restartButton = this.gameOverElement.querySelector<HTMLButtonElement>(
      '.game-over__restart',
    )

    if (restartButton === null) {
      throw new Error('Restart button could not be created.')
    }
    ...
    this.enemySystem = new EnemySystem(this.arena, (enemy) => {
      console.warn(
    ...
    this.explosionSystem = new ExplosionSystem(
      this.arena,
      (gridX, gridY) => {
        this.enemySystem.handleExplosionCell(gridX, gridY)
      },
    )
  ...
  private updateHealthHud(): void {
    ...
    this.healthHudElement.innerHTML = `
      <div class="health-hud__label">LIFE</div>
      <div class="health-hud__hearts">${hearts.join('')}</div>
    `

    if (this.player.isDead) {
      this.healthHudElement.classList.add('is-dead')
    } else {
      this.healthHudElement.classList.remove('is-dead')
    }
  }
  ...
  private showGameOver(): void {
    ...
    console.info('[Game] Game Over screen displayed.')
  }

  private readonly restart = (): void => {
    console.info('[Game] Restarting game.')
  ...
  private readonly animate = (): void => {
    ...
    if (!this.gameOverShown) {
      ...
      this.explosionSystem
        ...
        .forEach(({ gridX, gridY }) => {
          this.powerUpSystem.trySpawnPowerUp(gridX, gridY)
        })
