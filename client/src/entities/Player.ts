...
export class Player {
  ...
  public takeDamage(): void {
    if (this.isDead || this.invulnerabilityTimer > 0) {
      return
    }
    ...
    if (this.health <= 0) {
      this.health = 0
      this.isDead = true
      this.debugAnimation = 'Fire_DEATH'
      this.playDebugAnimation('Fire_DEATH')

      console.info('[Player] Player died.')
      return
    }

    this.debugAnimation = 'Fire_HIT'
    this.playDebugAnimation('Fire_HIT')
  }
  ...
  public collectPowerUp(type: PowerUpType): void {
    ...
    if (type === 'FIRE') {
      ...
      this.movementSpeed += SPEED_POWERUP_AMOUNT
    }
  }
  ...
  private addCharacterModel(
    ...
    this.logMissingAnimation('Fire_HIT')
    this.logMissingAnimation('Fire_DEATH')

    this.playAnimation(
      this.debugAnimation ?? (this.isMoving() ? 'Fire_WALK' : 'Fire_IDLE'),
    )
  }
  ...
  private tryMove(direction: MovementDirection): void {
    ...
    this.targetPosition.copy(
      this.arena.gridToWorld(nextGridX, nextGridY),
    )
  ...
  private updateMovement(deltaTime: number): void {
    ...
    this.movementProgress = Math.min(
      this.movementProgress + deltaTime * this.movementSpeed,
      1,
    )
